...
def start_local_weaviate() -> Optional[subprocess.Popen]:
    ...
    process = subprocess.Popen(
        [
            ...
            "--port", "8080",
            "--scheme", "http",
            "--persistence-data-path", "/tmp/weaviate-test-data",
        ],
...
class TestWeaviateStorageIntegration:
    ...
    def test_add_documents(self, weaviate_storage):
        ...
        count = weaviate_storage.add_documents(
            documents, document_id=test_doc_id, document_type=DocumentType.KNOWLEDGE
        )
    ...
    def test_add_chat_messages(self, weaviate_storage):
        ...
        count = weaviate_storage.add_chat_messages(
            messages, chat_id=test_chat_id, group_consecutive=False
        )
