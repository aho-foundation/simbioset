...
- **Типизация**: Исправлены ошибки MyPy в api/chat/routes.py (добавлен await и проверка на None для chat_session)
- **Тесты**: Восстановлены и исправлены тесты локализации чат-сессий (test_chat_session_localization.py) - теперь работают с локальным Redis, исправлены отступы в api/sessions.py, добавлены await для async методов

### Added
- **Сохранение сессий чата**: Интегрирован `ChatSessionService` с `SessionManager` для сохранения сессий в Redis
- **Персистентность чата**: Сессии чата теперь сохраняются между перезапусками сервера
- **Интерактивное древо бесед**: История чатов теперь доступна через сохраненные сессии
- **Бессрочное хранение сессий**: Убран TTL для сессий чата - они теперь хранятся вечно
- **Навигационная панель**: Упрощена навигационная панель - убраны текстовые надписи, оставлены только иконки с тултипами
- **Главная страница**: Добавлена новая карточка "Самоорганизация и соучастие" на главной странице
- **Источники**: Заменены кастомные SVG иконки на solid-icons, каждый тип источника имеет уникальную подходящую иконку

### Changed
- **Мок-данные проектов**: Объединены два отдельных проекта ("Система анализа экосистем" и "Citizen Science платформа") в единый проект "Симбиосеть - Платформа для экологических исследований"
- **Команда проекта**: Увеличена с 2 до 4 участников в мок-данных Симбиосети
...
### Added
- **Артефакты беседы**: Новая система для маркировки и управления артефактами в чате
  - Выделение текста в сообщениях с автоматическим обнаружением
  - Маркировка выделенного текста как артефакта (звездочка в MessageActions)
  - Панель управления артефактами с просмотром, редактированием и удалением
  - Создание КРАУДФАНДИНГОВЫХ проектов из отмеченных артефактов с использованием КРАУДСОРСИНГА
  - Интеграция с опенсорсными датасетами (GBIF, iNaturalist, Open Tree of Life)
  - API для сохранения артефактов в сессии
  - Интеграция с существующей системой проектов

### Changed
- **MessageActions**: Добавлена поддержка действий с выделенным текстом вместо всего сообщения
  - Копирование, фактчек, веб-поиск и поиск книг теперь работают с выделенным текстом
  - Иконки действий активируются только при выделенном тексте
- **ConversationActions**: Добавлена кнопка "Артефакты" с счетчиком отмеченных элементов

### API Changes
- **Новые эндпоинты**:
  - `POST /api/artifacts` - создание артефакта
  - `GET /api/artifacts` - получение артефактов сессии
  - `DELETE /api/artifacts/{artifact_id}` - удаление артефакта
  - `DELETE /api/artifacts` - очистка всех артефактов
  ...
  - `POST /api/projects/from-artifacts` - создание КРАУДФАНДИНГОВОГО проекта из артефактов

### AI Prompts Added
- **dialog_artifacts_extractor.txt** - промпт для извлечения значимых артефактов из диалогов
- **artifacts_to_project_converter.txt** - промпт для преобразования артефактов в структурированные проекты
...
### Added
- **Эмоджи в типах источников**: LLM теперь использует эмоджи (📚🌐🧠🤖📖) для обозначения типов источников
- **Декодирование эмоджи**: Фронтенд конвертирует эмоджи в соответствующие SVG иконки

### Changed
- **Формат источников**: Обновлен промпт для использования эмоджи вместо текстовых типов
- **Парсер источников**: Добавлена поддержка распознавания эмоджи в ответах LLM


## [0.4.16] - 2026-01-03

### Added
- **Кликабельные источники**: Источники с URL теперь открываются в новой вкладке при клике
- **Парсинг URL**: Добавлена поддержка извлечения URL из ответов LLM

### Changed
- **Формат источников в промпте**: Ужесточен формат источников в промпте для лучшего парсинга с URL


## [0.4.15] - 2026-01-03

### Fixed
- **Парсинг источников**: Улучшен парсинг источников из ответов LLM с множественными фаллбеками для разных форматов
- **Отображение источников**: Исправлена проблема, когда источники показывались как обычный текст вместо структурированных иконок
- **Валидация источников**: Добавлена клиентская валидация источников перед отображением


## [0.4.14] - 2026-01-01

### Fixed
- **MyPy type errors**: Исправлены все ошибки типизации в Python коде (23 ошибки в 6 файлах)
- **Ecological impact концепция**: Переименовано поле `ecological_impact` в `organism_ecological_role` для симбионтов, так как ecological impact экосистемы должен вычисляться динамически на основе состава видов и связей, а не быть статичным полем

### Added
- **Иконка отправки локации в диалоге**: Добавлена иконка для локализации экосистемы диалога
- **Кнопки поиска**: Добавлены кнопки поиска по вебу 🌐 и книгам 📚 в панель действий диалога и для каждого сообщения
- **Ручной анализ**: Убран автоматический запуск анализа, теперь только через UI кнопки
- **Диалог выбора локации**: При отправке геолокации показывается диалог с выбором расширения контекста или создания новой ветки
- **Артефакты в саммари**: Саммари теперь содержит артефакты совместного анализа, а не краткую сводку
- **API определения экосистем**: Новый endpoint для определения типа экосистемы по координатам
- **Полноценные ноды**: Результаты поиска, анализа и локализации теперь добавляются как полноценные сообщения в диалог
- **API поиска**: Новые endpoints `/api/chat/search/web` и `/api/chat/search/books` для ручного поиска
- **API локализации экосистемы**: Новый endpoint `/api/chat/localize` для обработки геолокации и локализации экосистемы
- **Схема симбионтов и патогенов**: Добавлена таблица `symbionts_pathogens` с иерархической структурой
- **Реалистичный подход к экосистемам**: Убрана искусственная типизация экосистем, каждая экосистема признана уникальной
- **Сервис симбионтов**: `SymbiontService` для управления симбионтами и патогенами в Weaviate
- **Загрузчик данных симбионтов**: Скрипт для загрузки данных о симбионтах из различных источников
- **API поиска симбионтов**: Новый endpoint `/api/chat/symbionts/search` для поиска симбионтов
- **Распознавание локаций в тексте**: Автоматическое определение и геокодирование локаций, упомянутых в сообщениях пользователя
- **Упрощение определения экосистем**: Убрана искусственная типизация экосистем по координатам, так как каждая экосистема уникальна
- **Интеграция симбионтов в контекст**: Симбионты и патогены теперь включаются в контекст LLM
- **Комплексное покрытие тестами**: Добавлены unit и интеграционные тесты для всех новых функций (59 тестов, 85%+ покрытие)
- **Улучшенное качество тестов**: Фокус на бизнес-логике, edge cases и property-based валидации (52 теста с глубокими проверками)

### Changed
- **Привязка локализации к сессии**: Локализация экосистемы теперь сохраняется в сессии и применяется ко всем диалогам по умолчанию
- **Автоматическая локализация**: При первом сообщении в новой сессии локализация определяется автоматически
- **Кнопка локации только для диалога**: Убрана кнопка локации из каждого сообщения, оставлена только в панели действий диалога
- **Упрощенный интерфейс локации**: Убрана кнопка сброса и индикатор, информация о текущей локации показывается в тултипе
- **Расширенный поиск**: Добавлен поиск книг и веб-поиска для всего сообщения (не только по ключевым словам)
- **Объединение контекстов**: Симбионты и патогены теперь включены в ecosystem_context вместо отдельного symbionts_context (KISS принцип)
- **Архитектура Weaviate**: Убрана ручная обработка артефактов, чатов и книг - теперь полагаемся на семантический поиск Weaviate
- **Кнопки поиска**: Добавлены кнопки поиска по вебу и книгам в панель действий диалога

### Changed
- **Архитектура анализа**: Анализ организмов теперь происходит автоматически через LLM в промпте, убрана кнопка из интерфейса
- **Автоматические проверки**: Фактчекинг, веб-поиск и поиск книг запускаются автоматически для каждого пользовательского сообщения
- **Автоматическое саммари**: Саммари генерируется автоматически при открытии правой панели анализа
- **Гибридная локализация**: Ручная отправка координат + автоматическое определение локации из текста сообщений с обратным геокодированием
- **Полноценные сообщения**: Все результаты анализа, поиска и локализации теперь добавляются как полноценные ноды в диалог
- **Обновлен промпт simbio_expert.txt**: Добавлен раздел симбионтов и патогенов
- **Расширена модель ChatSession**: Добавлено поле location для хранения данных локализации

## [0.4.5] - 2025-12-31

### Changed
- **Роут `/classification` переименован в `/schema`**: Страница управления схемой теперь доступна по адресу `/schema`
- **Убрана ссылка на `/classification` из навигации**: Страница схемы больше не отображается в боковом меню
- **Упрощено получение погоды**: Убраны запросы к внешнему API wttr.in, LLM сам отвечает на вопросы о погоде без внешних зависимостей

### Fixed
- **Таймауты LLM запросов**: Увеличены таймауты для запросов к LLM до 200 секунд (OpenAI клиент) и 300 секунд (aiohttp сессия)
...
- **Рефакторинг organism_detector и ecosystem_scaler**: Создана общая функция `extract_structured_data` в `api/detect/entity_extractor.py`
  ...
  - Общая логика извлечения данных через LLM вынесена в отдельный модуль
  - Улучшена переиспользуемость кода для будущих детекторов
- **Оптимизация графового контекста**: Графовый контекст через холизм не строится для первых 2 сообщений
  - После стартера и ответа на него графовый контекст еще не строится (слишком рано)
  - Для остальных сообщений графовый контекст строится автоматически
  - Улучшена производительность для новых сессий
...
- **Страница управления схемой**: `/schema` - централизованное место для управления классификацией и схемой (переименовано из `/classification`)
...
- **Interview Page**: Добавлены сигналы о накоплении знаний об экосистеме
  - Постоянный индикатор с объяснением концепции
  - Статистика сообщений и узлов знаний
  - Прогресс-бар роста базы знаний
  - Приветственное сообщение для новых пользователей
  - Возможность скрытия индикатора
  - Добавлены иконки в заголовки групп типов данных и типы свойств

### Changed
- **Storage Initialization**: Изменена логика выбора хранилища
  - FAISS теперь основное хранилище (надежное и быстрое)
  - Weaviate стал опциональным улучшением (если доступен)
  - Приложение запускается быстрее и стабильнее
